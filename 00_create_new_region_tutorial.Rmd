...
4. Write the dataframe to an *fst* file and store it in the `data/region/` subfolders following the filenaming scheme `data/region/region_sensor_variable_year.fst`. Replace "region", "sensor", "variable", and "year" with the appropriate values (see example below). Note that this step (and the app) requires the "fst" library in R.  
