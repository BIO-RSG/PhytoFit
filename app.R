...
# shiny, leaflet, dataframes, polygons, projections, etc. 

# TROUBLESHOOTING STEP 1:
# Clear environment:
#       In the console, type rm(list=ls())
# Restart R:
#       From RStudio, select Session --> Restart R
...
#**********
# reactiveValues():
# "input" is a list of reactive values, but creating another reactiveValues list
# (the "state" list in this app) gives you more control over what user input
# will trigger which functions, and whether or not the values in stats/plots/etc
# use the current user (widget) input.
# EXAMPLE specific to this app:
...
# actionButton(inputId, label, width,
#              style = "white-space: normal;")

# ERROR MESSAGE:
#   Error in .getReactiveEnvironment()$currentContext() : 
#       Operation not allowed without an active reactive context. (You tried to do something that can only be done from inside a reactive expression or observer.)
# This means you tried to do something with a reactiveValue (for example, the
# "input" or "state" variables in this app) outside of a reactive expression
# like reactive, observe, eventReactive, or observeEvent - actions on
# reactiveValues must be within one of those (or a "render" function).
...
# Make shiny wait between character strokes in a numericInput or textInput bar
# using "debounce" on a reactive function.
...
# For multi-column group of input checkboxes:
...
# Add multiple polygons to a map with the sp package, lapply, and one instance of
# addPolygons when creating the base leaflet map
...
# Use leaflet and leafletProxy to create a background map and update it later
# without recreating the whole map
# https://rstudio.github.io/leaflet/shiny.html

# Instead of using raster, use circle markers and plot the binned data straight
# to the map. Much slower, but here are tips to speed it up:
...
# Saving a leaflet map that uses leafletProxy() to update is too advanced to
# figure out right now (mapshot, leaflet-easyprint not working), so instead I'm
# using downloadButton in the UI, corresponding downloadHandler in the server,
# and saveWidget within downloadHandler (or ggsave or write.csv for the plots
# and csv files) to allow the user to save the map widget in its current state
# just by clicking a download button.
...
# BUT...
# If you use addMapPane, the map is saved with "SVG" instead of on a canvas
# i.e. the "preferCanvas" option does nothing
# preferCanvas significantly speeds it up when plotting thousands of points,
# so this is more important than addMapPane
...
# Programmatically delete drawn polygons (instead of making the user delete them
# with the draw toolbar):
# https://github.com/bhaskarvk/leaflet.extras/issues/96

# March 2020: Tried using patchwork for the output plots, but the bloomfit click
# didn't work after this. Forgot to copy links but very briefly skimmed one where
# a user said it would not work. Possibly something to do with how patchwork prints
# plots (or doesn't print them...?) but I don't understand how that works, so went
# back to the old method for now. (The purpose of this patchwork detour was to
# find a better way to display statistics/parameter tables next to the plots)
...
ui <- fluidPage(
    ...
    # Horizontal line styling for sidebar hr
    tags$head(
        tags$style(HTML(
    ...
    # Remove drawn polygons programmatically
    tags$div(tags$script(HTML(
